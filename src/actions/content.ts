...
import { getAllContentItems } from '@/lib/db/repository';
import type { ContentItem } from '@/types/content';

// コンテンツアイテムを取得するサーバーアクション
export async function getContentItems(): Promise<ContentItem[]> {
  try {
    // Tursoデータベースからデータを取得
    return await getAllContentItems();
  } catch (error) {
    console.error('Failed to fetch content items from database:', error);
    // フォールバックとして静的データを返す
...
// フォールバック用のデフォルトデータ
function getDefaultContentItems(): ContentItem[] {
  return [
    {
      id: '1',
      type: 'release',
    ...
    },
    {
      ...
      description:
        'Webサービスをリリースする前に確認したい項目をまとめました。',
      date: 'Jul 4',
